// State Management - قائمة القراء الشاملة والمضمونة
const RECITERS = [
...
async function loadSurah(surahNum, autoPlayAfterLoad = false) {
  ...
  currentSurahInfo.innerText = ''; // إخفاء الوصف الإنجليزي والنوع وعدد الآيات من الهيدر
  ...
  try {
    ...
    if (selectedReciterId === 'banna_hf') {
      // التعامل المخصص للشيخ محمود علي البنا عبر روابط EveryAyah
      ...
      ayahsData = arabic.map((a) => {
        ...
        const audioUrl = `https://www.everyayah.com/data/mahmoud_ali_al_banna_32kbps/${sStr}${aStr}.mp3`;
      ...
      // باقي القراء من الـ API الأساسي
...
function playAyah(index) {
  ...
  audio.play().then(() => {
    ...
    // تجهيز الآية التالية مسبقاً في الخلفية لتفادي أي تأخير
